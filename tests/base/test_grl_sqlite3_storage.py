import pytest
from pygrl import SQLite3_Storage, GeneralRateLimiter as grl
import time
from typing import Any


STORAGE = SQLite3_Storage(db_path="./test_grl_sqlite3_storage.db", table_name="storage", overwrite=True)
...
def test_grl_ss_check_limit_w_same_key(max_requests: int, time_window: int):
    # Parameter
    key = "key"
...
@pytest.mark.parametrize("max_requests,time_window", [(3, 3), (10, 5), (5, 4)])
def test_grl_ss__call__w_same_key(max_requests: int, time_window: int):
    # Parameter
    key = "key"
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    t1 = time.time()
...
def test_grl_ss_check_limit_w_different_key(
    ...
    for _ in range(max_requests):
        ...
        for key_index in range(number_of_key):
            assert rate_limiter.check_limit(key_index)
    ...
    while time.time() - start < time_window - buffer:
        ...
        for key_index in range(number_of_key):
            assert not rate_limiter.check_limit(key_index)
...
def test_grl_ss__call__w_different_key(
    max_requests: int, time_window: int, number_of_key: int
):
    # Variable
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    ...
    while time.time() - start < time_window - buffer:
        ...
        # Otherwise, rate limiting of the last few keys might be lifted, thus fail the test
        for key_index in range(number_of_key):
            assert not rate_limiter(key_index)
        time.sleep(delay)  # Force a short sleep to reduce resource allocation
    time.sleep(buffer)  # Ensure the time_window is reached
    # Expect rate limits to be lifted
    for key_index in range(number_of_key):
...
def test_grl_ss_check_limit_w_different_key_sequence(
    ...
    for k in keys:
        assert rate_limiter.check_limit(k)
    # Test
    assert rate_limiter.check_limit(key) is expected
    # Runtime check
    diff = time.time() - t1
    if diff > time_window:
        raise RuntimeError("Previous steps took longer than the time window")
...
def test_grl_ss__call__w_different_key_sequence(
    max_requests: int, time_window: int, keys: list, key: Any, expected: bool
):
    # Variable
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    ...
    # Pre-load the rate limiter with keys
    for k in keys:
        assert rate_limiter(k)
    # Test
    assert rate_limiter(key) is expected
    # Runtime check
    diff = time.time() - t1
    if diff > time_window:
        raise RuntimeError("Previous steps took longer than the time window")
...
def test_grl_ss_check_limit_reset(max_requests: int, time_window: int):
    # Parameter
    key = "key"
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    t1 = time.time()
...
@pytest.mark.parametrize("max_requests,time_window", [(3, 3), (4, 3), (10, 4)])
def test_grl_ss__call__reset(max_requests: int, time_window: int):
    # Parameter
    key = "key"
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    t1 = time.time()
