import pytest
from pygrl import SQLite3_Storage, GeneralRateLimiter_with_Lock as grl
import time
from typing import Any


STORAGE = SQLite3_Storage(db_path="./test_grlwl_sqlite3_storage.db", table_name="storage", overwrite=True)
...
async def test_grlwl_ss_check_limit_w_same_key(max_requests: int, time_window: int):
    # Parameter
    key = "key"
...
@pytest.mark.asyncio
@pytest.mark.parametrize("max_requests,time_window", [(3, 3), (10, 5), (5, 4)])
async def test_grlwl_ss__call__w_same_key(max_requests: int, time_window: int):
    # Parameter
    key = "key"
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    t1 = time.time()
...
async def test_grlwl_ss_check_limit_w_different_key(
...
async def test_grlwl_ss__call__w_different_key(
    max_requests: int, time_window: int, number_of_key: int
):
    # Variable
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
...
async def test_grlwl_ss_check_limit_w_different_key_sequence(
...
async def test_grlwl_ss__call__w_different_key_sequence(
    max_requests: int, time_window: int, keys: list, key: Any, expected: bool
):
    # Variable
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
...
async def test_grlwl_ss_check_limit_reset(max_requests: int, time_window: int):
    # Parameter
    key = "key"
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    t1 = time.time()
...
@pytest.mark.asyncio
@pytest.mark.parametrize("max_requests,time_window", [(3, 3), (4, 3), (10, 4)])
async def test_grlwl_ss__call__reset(max_requests: int, time_window: int):
    # Parameter
    key = "key"
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    t1 = time.time()
...
async def test_grlwl_check_limit_sequence_info(
    ...
    # Parameter
    rate_limiter = grl(STORAGE, max_requests=max_requests, time_window=time_window)
    # Load the sequence into the rate limiter
