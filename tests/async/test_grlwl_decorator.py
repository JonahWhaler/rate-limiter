...
from pygrl import (
    ...
    SQLite3_Storage,
    Storage,
    GeneralRateLimiter_with_Lock as grl,
...
async def test_decorated_function_wo_key(
...
async def test_decorated_function_w_same_key(
    storage: Storage, max_requests: int, time_window: int
):
    # Define a decorated function
    @grl.general_rate_limiter(storage, max_requests, time_window)
    async def function(**kwargs):  # Add **kwargs to accept `key` as keyword argument
        return True

    t1 = time.time()
    # Call the function up to the allowed times (`max_requests`)
    for _ in range(max_requests):
        await function(key="key")
    # Expect it to fail at max_requests + 1 time
    with pytest.raises(ExceededRateLimitError) as exc_info:
        await function(key="key")
    # Runtime check
...
async def test_decorated_function_w_different_key(
    ...
    for _ in range(max_requests):
        ...
        for key_index in range(number_of_key):
            await function(key=f"key:{key_index}")
...
async def test_decorated_function_w_different_key_sequence(
    storage: Storage,
    max_requests: int,
    time_window: int,
    key_sequence: list,
    ...
    for k in key_sequence:
        await function(key=f"k:{k}")
    # Test
    with pytest.raises(ExceededRateLimitError) as exc_info:
        await function(key=f"k:{key}")
    # Runtime check
...
async def test_decorated_function_w_key_builder_kwargs(storage: Storage, kwargs: dict, key_builder: Callable):
    # Parameter
    max_requests = 3
    time_window = 3
    ...
    await function(hello="world", my="friend", username="someone", department="General")
    # Runtime check
    diff = time.time() - start
    if diff > time_window:
        raise RuntimeError("Previous steps took longer than the time window")
...
async def test_decorated_function_w_key_builder_args(storage: Storage, args: tuple, key_builder: Callable):
    # Parameter
    max_requests = 3
    time_window = 3
